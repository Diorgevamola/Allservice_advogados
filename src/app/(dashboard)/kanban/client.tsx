...
export default function KanbanClient({ initialLeads }: { initialLeads: Lead[] }) {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case "Desqualificado": return "bg-red-500 text-white";
        ...
        >
            <DragDropContext onDragEnd={onDragEnd}>
                {Object.values(columns).map((column) => (
                    ...
                    >
                        <div className={`flex flex-col gap-1 mb-3 p-3 rounded-t-xl bg-card backdrop-blur-md border-b border-border sticky top-0 z-10 shadow-sm`}>
                            <div className="flex items-center justify-between">
                                <h3 className="font-light text-lg flex items-center gap-2 text-foreground tracking-tight">
                                    {column.title}
                                </h3>
                        ...
                        <Droppable droppableId={column.id}>
                            {(provided, snapshot) => (
                                <div
                                    ...
                                    className={`flex-1 p-2 rounded-xl transition-colors min-h-[500px] ${snapshot.isDraggingOver ? "bg-accent/50" : "bg-muted/20"
                                        }`}
                                >
                                            ...
                                            {(provided, snapshot) => (
                                                ...
                                                >
                                                    <Card
                                                        ...
                                                        className={`group relative overflow-hidden p-4 border border-border bg-card/50 backdrop-blur-md hover:bg-accent/50 transition-all duration-300 cursor-pointer ${snapshot.isDragging ? "shadow-xl border-primary scale-105 z-50" : "shadow-sm hover:shadow-md"
                                                            }`}
                                                    >
                                                        ...
                                                        <div className="pl-2 space-y-3">
                                                            <div className="flex items-start justify-between">
                                                                <span className="font-light text-foreground group-hover:text-primary line-clamp-1 flex items-center gap-2 text-base transition-colors">
                                                            ...
                                                            <div className="flex items-center gap-2 text-sm text-muted-foreground font-light group-hover:text-primary/70 transition-colors">
                                                                <Phone className="w-3.5 h-3.5 opacity-70" />
                                                                {lead.telefone}
                                                            </div>

                                                            <div className="flex items-center justify-between pt-3 border-t border-border transition-colors">
                                    ...
                                    {column.leads.length > visibleCounts[column.id] && (
                                        <div
                                            ...
                                            className="h-20 flex items-center justify-center text-muted-foreground text-xs animate-pulse font-light"
                                        >
                        ...
                        <>
                            <SheetHeader className="p-8 pb-6 border-b border-border flex-shrink-0">
                                <div className="flex items-center gap-4 mb-4">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white text-xl font-light bg-gradient-to-br from-primary to-primary/80 border border-primary/20 shadow-lg shadow-primary/20`}>
                                        {selectedLead.nome?.[0] || "?"}
                                    </div>
                                    <div>
                                        <SheetTitle className="text-2xl font-light text-foreground tracking-tight">
                                            {selectedLead.nome || "Lead Sem Nome"}
                                        </SheetTitle>
                                        <div className="flex items-center gap-2 text-muted-foreground font-light mt-1 text-sm">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <Badge variant="secondary" className={`${getStatusColor(selectedLead.Status)} border-none py-2 px-4 rounded-xl text-xs uppercase tracking-widest font-semibold shadow-sm`}>
                                        {selectedLead.Status}
                                    </Badge>
                                </div>
                            </SheetHeader>
                                ...
                                <div className="space-y-8 pb-10">
                                    ...
                                    <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/50 border border-border">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="text-sm font-medium text-foreground">Respostas de IA</div>
                                                <div className="text-xs text-muted-foreground font-light">Lead sendo atendido pelo robô</div>
                                            </div>
                                        </div>
                                        <Switch
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-muted-foreground text-[10px] uppercase tracking-widest font-semibold">Resumo da Conversa</Label>
                                        <div className="p-4 rounded-2xl bg-muted/30 border border-border text-sm font-light leading-relaxed text-foreground">
                                    ...
                                    <div className="space-y-4">
                                        <Label className="text-muted-foreground text-[10px] uppercase tracking-widest font-semibold">Etapas do Funil</Label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {[...Array(12)].map((_, i) => {
                                                const key = `t${i + 1}` as keyof Lead
                                                const value = selectedLead[key as keyof Lead]
                                                return (
                                                    <div key={key} className={`p-3 rounded-xl border ${value ? 'border-primary/30 bg-primary/5 text-primary' : 'border-border bg-muted/50 text-muted-foreground opacity-60'} transition-all`}>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pb-4">
                                        <div className="space-y-1">
                                            <div className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Criado em</div>
                                            <div className="text-sm font-light text-foreground">
                                                {format(new Date(selectedLead.created_at), "dd MMMM yyyy", { locale: ptBR })}
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <div className="text-[10px] text-muted-foreground uppercase tracking-widest font-medium">Horário</div>
                                            <div className="text-sm font-light text-foreground">
